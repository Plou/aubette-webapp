...
function composition() {
  ...
  nLinesX = Math.floor((Math.random() * 10) + 5);
  nLinesY = Math.floor((Math.random() * 6) + 3);
  x = [];
  for (i = 0, t = nLinesX; i < t; i++) {
    // require lines to be spaced apart by at least 10 pixels
    x.push((Math.round((Math.random() * (canvas.height / 10)) - 10) + 1) * 10)
  };
  y = [];
  for (i = 0, t = nLinesY; i < t; i++) {
    // require lines to be spaced apart by at least 10 pixels
    y.push((Math.round((Math.random() * (canvas.width / 6)) - 10) + 1) * 10)
  };
  //x = [150]; //for testing
  //y = [50]; //for testing
  x = x.sort();
  ...
  for (j = 0; j < y.length; j++) {
    ...
    context.lineTo(canvas.width, y[j]);
    context.stroke();
    context.lineWidth = 4;
    context.strokeStyle = 'black';
    context.stroke();
  ...
  nColors = Math.floor((Math.random() * (15)) + 4);
  nColors = nLinesX * nLinesY;
  for (c = 0; c < nColors; c++) {
...
module.exports = { 'compose': composition, 'clear': clearCanvas };
